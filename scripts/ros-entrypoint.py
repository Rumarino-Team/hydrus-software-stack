...
class HydrusROSManager:
    ...
    def _run_command(self, cmd: List[str], check: bool = True, capture_output: bool = False, 
    ...
    def _clean_simulator_on_arm(self):
        ...
        if arch == "aarch64":
            print("Detected ARM architecture (Jetson or similar), deleting simulator folder...")
    ...
    def _start_virtual_arduino(self):
        ...
        virtual_arduino_script = self.ros_dir / "src/hydrus-software-stack/virtual_arduino.py"
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                result = self._run_command([
                    "arduino-cli", "upload", "-p", port, "--fqbn", arduino_board, "Hydrus.ino"
                ], check=False, capture_output=True, cwd=arduino_dir)
    ...
    def _setup_serial_monitoring(self):
        ...
        # Start tmux sessions
        tmux_script = self.ros_dir / "src/hydrus-software-stack/start_tmux_sessions.py"
        if tmux_script.exists():
            ...
            tmux_script = self.ros_dir / "src/hydrus-software-stack/start_tmux_sessions.sh"
            if tmux_script.exists():
    ...
    def _start_rosbag_playback(self):
        ...
        print("ROSBAG playback is enabled. Looking for rosbag files in /rosbags directory...")
        ...
        if rosbags_dir.exists():
            ...
            if bag_files:
                ...
                print("No rosbag files found in /rosbags directory. Skipping rosbag playback.")
        else:
    ...
    def _start_rviz(self):
        ...
        rviz_config_path = self.ros_dir / "src/hydrus-software-stack/autonomy/config" / self.rviz_config
    ...
    def main(self):
        ...
        try:
            ...
            if self.deploy:
                ...
                print("Deploy is not set or is set to false. Skipping deployment setup.")
