...
class CameraEntrypoint:
    ...
    def _setup_environment(self) -> Dict[str, str]:
        ...
        if setup_file.exists():
            ...
            env["ROS_PACKAGE_PATH"] = f"{self.workspace_dir}/src:{env.get('ROS_PACKAGE_PATH', '')}"
            env["CMAKE_PREFIX_PATH"] = f"{self.workspace_dir}/devel:{env.get('CMAKE_PREFIX_PATH', '')}"
            env["LD_LIBRARY_PATH"] = f"{self.workspace_dir}/devel/lib:{env.get('LD_LIBRARY_PATH', '')}"
            env["PYTHONPATH"] = f"{self.workspace_dir}/devel/lib/python3/dist-packages:{env.get('PYTHONPATH', '')}"
