...
class HydrusTmuxManager:
    ...
    def _setup_virtual_arduino(self):
        ...
        virtual_arduino_script = self.catkin_ws / "src/hydrus-software-stack/scripts/virtual_arduino.py"
    ...
    def _setup_serial_monitor(self):
        ...
        if not Path("/tmp/hydrus_serial/catpid.txt").exists():
            ...
            setup_script = self.catkin_ws / "src/hydrus-software-stack/scripts/setup_serial_monitor.py"
            if setup_script.exists():
                try:
                    subprocess.run([sys.executable, str(setup_script), "/dev/ttyACM0"], check=False)
                except Exception as e:
                    print(f"Failed to setup serial monitor: {e}")
            else:
                # Fallback to bash script
                setup_script = self.catkin_ws / "src/hydrus-software-stack/scripts/setup_serial_monitor.sh"
                if setup_script.exists():
                    subprocess.run(["bash", str(setup_script), "/dev/ttyACM0"], check=False)
            time.sleep(2)
    
    def _kill_existing_session(self):
        """Kill existing tmux session if it exists"""
        try:
            self._run_command(["tmux", "kill-session", "-t", "hydrus"], check=False, capture_output=True)
        except:
            pass
    ...
    def _create_controls_window(self):
        ...
        self._tmux_command([
            "new-session", "-d", "-s", "hydrus", "-n", "Controls"
        ])
    ...
    def _create_arduino_window(self):
        ...
        setup_cmd = f"bash {self.catkin_ws}/src/hydrus-software-stack/setup_serial_monitor.sh"
        if (self.catkin_ws / "src/hydrus-software-stack/setup_serial_monitor.py").exists():
        ...
        monitor_cmd = f"bash {self.catkin_ws}/src/hydrus-software-stack/monitor_arduino_logs.sh"
        if (self.catkin_ws / "src/hydrus-software-stack/monitor_arduino_logs.py").exists():
    ...
    def _create_cv_window(self):
        ...
        try:
            ...
            self._tmux_command([
                "select-pane", "-t", "hydrus:2.2"
            ])
            self._tmux_command([
                "resize-pane", "-x", str(quarter_cols)
            ])
        except:
    ...
    def _create_mission_planner_window(self):
        ...
        mission_manager_cmd = f"{source_cmd} && roslaunch autonomy mission_planner.launch"
        
        self._tmux_command([
            "send-keys", "-t", "hydrus:3.0",
    ...
    def main(self):
        ...
        try:
            ...
            print(f"Tmux session 'hydrus' created with catkin workspace at {self.catkin_ws}.")
