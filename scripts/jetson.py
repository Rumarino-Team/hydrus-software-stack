...
class JetsonDeploymentManager:
    ...
    def _setup_architecture(self):
        ...
        if arch == "x86_64":
            ...
            try:
                self._run_command(["qemu-system-aarch64", "--version"], capture_output=True)
            except (subprocess.CalledProcessError, FileNotFoundError):
                print("ERROR: QEMU is not installed. Please install it with:")
                print("  sudo apt-get install qemu-system-arm qemu-efi qemu-user-static")
                sys.exit(1)
    ...
    def _build_and_run_zed_camera(self):
        ...
        if self.use_qemu:
            ...
            build_cmd = ["docker", "build"] + self.qemu_args + [
                "-t", "zed-camera", "-f", dockerfile_path, "."
            ]
        else:
            print("Building ZED camera container...")
            build_cmd = ["docker", "build", "-t", "zed-camera", "-f", dockerfile_path, "."]
    ...
    def _build_and_run_hydrus(self):
        ...
        try:
            result = self._run_command([
                "docker", "images", "hydrus:latest", "--format", "{{.Repository}}:{{.Tag}}"
            ], capture_output=True)
            ...
            if "hydrus:latest" not in result.stdout:
                ...
                if self.use_qemu:
                    ...
                    build_cmd = ["docker", "build", "-t", "hydrus:latest", "-f", dockerfile_path, "."]
