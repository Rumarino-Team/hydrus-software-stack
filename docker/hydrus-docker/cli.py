  ...
  python3 run_docker.py --development --force-cpu --no-debug-arduino
        ...
        for group_name, group_info in CONFIGURATION_GROUPS.items():
            group_parser.add_argument(
                f"--{group_name}", action="store_true", help=group_info["description"]
            )

        # Deployment type arguments
        deployment_group = parser.add_argument_group("Deployment Type")
        deployment_group.add_argument(
            "--force-cpu", action="store_true", help="Force CPU deployment"
        )
        deployment_group.add_argument(
            "--cuda", action="store_true", help="Force CUDA/GPU deployment"
        )
        deployment_group.add_argument(
        ...
        config_group.add_argument(
            "--zed", action="store_true", help="Enable ZED camera display in RViz"
        )
        config_group.add_argument(
            "--rosbag", action="store_true", help="Enable rosbag playback"
        )
        config_group.add_argument(
            "--download-rosbag",
            action="store_true",
            help="Auto-download rosbag files if missing",
        )
        config_group.add_argument(
            "--debug-arduino",
            action="store_true",
        ...
        config_group.add_argument(
            "--no-zed", action="store_true", help="Disable ZED camera display"
        )
        config_group.add_argument(
            "--no-rosbag", action="store_true", help="Disable rosbag playback"
        )
        config_group.add_argument(
            "--no-debug-arduino", action="store_true", help="Disable Arduino debugging"
        )
        ...
        vscode_group.add_argument(
            "--vscode",
            action="store_true",
            help="Open VS Code and connect to container",
        )
        vscode_group.add_argument(
            "--install-vscode-extensions",
            action="store_true",
            help="Auto-install missing VS Code extensions",
        )
        vscode_group.add_argument(
            "--no-vscode", action="store_true", help="Disable VS Code integration"
        )
        ...
        parser.add_argument(
            "--detach",
            action="store_true",
            help="Run containers in detached mode (background) instead of replacing terminal",
        )
    ...
    def validate_arguments(args):
        ...
        if sum(deployment_flags) > 1:
            raise ValueError(
                "Cannot specify multiple deployment types (--force-cpu, --cuda, --force-jetson)"
            )

        # Check for multiple configuration groups
        active_groups = [
            group
            for group in CONFIGURATION_GROUPS.keys()
