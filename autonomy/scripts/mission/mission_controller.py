...
class MissionController:
    ...
    def setup_windows(self):
        ...
        cmd_height = 2
        self.cmd_win = curses.newwin(
            cmd_height, self.width, self.height - cmd_height, 0
        )
        ...
        self.status_win = curses.newwin(
            content_height, status_width, header_height, status_x
        )

        # Details window (covers the whole content area when showing details)
        self.details_win = curses.newwin(
            content_height, content_width, header_height, 0
        )
    ...
    def connect_to_ros(self):
        ...
        try:
            ...
            self.select_service = rospy.ServiceProxy(
                "/mission_manager/select_mission", SetBool
            )
            ...
            self.show_message(
                f"ERROR: Failed to connect to mission manager services: {e}"
            )
    ...
    def status_callback(self, msg):
        ...
        try:
            ...
            if list(self.mission_status.keys()) != self.available_missions:
                ...
                if self.available_missions and self.selected_index >= len(
                    self.available_missions
                ):
    ...
    def generate_details_content(self):
        ...
        self.details_content.append(
            (
                f"Active Mission: {self.active_mission or 'None'}",
    ...
    def draw_mission_list(self):
        ...
        if not self.available_missions:
            self.list_win.addstr(
                2, 2, "No missions available", curses.color_pair(COLOR_WARNING)
            )
        else:
            for i, mission in enumerate(self.available_missions):
                ...
                if 2 + i < self.list_win.getmaxyx()[0] - 1:
                    self.list_win.addstr(
                        2 + i, 1, mission_text, curses.color_pair(color) | attr
                    )
    ...
    def draw_status(self):
        ...
        self.status_win.addstr(
            line, 2, "Active Mission:", curses.color_pair(COLOR_NEUTRAL)
        )
        line += 1

        if self.active_mission:
            self.status_win.addstr(
                line, 4, self.active_mission, curses.color_pair(COLOR_ACTIVE)
            )
        else:
        ...
        if self.active_mission and self.active_mission in self.mission_status:
            ...
            self.status_win.addstr(
                line, 10, status_text, curses.color_pair(status_color)
            )
            line += 2
            ...
            if "completion_percentage" in mission_data:
                ...
                self.status_win.addstr(
                    line, 2, "Progress:", curses.color_pair(COLOR_NEUTRAL)
                )
                line += 1
                ...
                # Show percentage
                self.status_win.addstr(
                    line, 2, f"{percentage:.1f}%", curses.color_pair(COLOR_NEUTRAL)
                )
                line += 2

            # Show other mission data
            self.status_win.addstr(
                line, 2, "Mission Data:", curses.color_pair(COLOR_NORMAL)
            )
            line += 1
            ...
            for key, value in mission_data.items():
                if key not in ["active", "completion_percentage"]:
                    ...
                    self.status_win.addstr(
                        line, 4, f"{key}: {value_str}", curses.color_pair(color)
                    )
                    line += 1
        else:
            self.status_win.addstr(
                line, 2, "No active mission selected", curses.color_pair(COLOR_WARNING)
            )
    ...
    def draw_details(self):
        ...
        if self.details_scroll > 0:
            self.details_win.addstr(
                1,
                self.details_win.getmaxyx()[1] - 3,
                "↑",
    ...
    def select_mission(self):
        ...
        try:
            ...
            if response and response.success:
                ...
                self.show_message(
                    f"ERROR: {response.message if response else 'Unknown error'}"
                )
