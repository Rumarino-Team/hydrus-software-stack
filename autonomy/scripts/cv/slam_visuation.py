...
class MonocularSLAM:
    ...
    def __init__(self, camera_matrix=None, dist_coeffs=None):
        ...
        # ORB detector
        self.orb = cv2.ORB_create(nfeatures=3500)  # Reduced feature count to improve performance
        ...
        self.max_trajectory_len = 500  # Reduced trajectory length to avoid memory issues
    ...
    def estimate_pose(self, pts1, pts2):
        ...
        try:
            ...
            if E is None or E.shape != (3, 3):
                print(f"Failed to compute valid essential matrix, shape: {E.shape if E is not None else 'None'}")
                return None, None, None
    ...
    def update_trajectory_image(self):
        ...
        cv2.line(self.trajectory_image, (self.center_x, self.center_y), 
                 (self.center_x + 50, self.center_y), (0, 0, 255), 2)  # X-axis (red)
        cv2.line(self.trajectory_image, (self.center_x, self.center_y), 
                 (self.center_x, self.center_y - 50), (0, 255, 0), 2)  # Y-axis (green)
        
        # Add text labels for axes
        cv2.putText(self.trajectory_image, "X", (self.center_x + 55, self.center_y + 15), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        cv2.putText(self.trajectory_image, "Y", (self.center_x - 15, self.center_y - 55), 
        ...
        if len(self.trajectory_history) > 1:
            ...
            for pos in self.trajectory_history:
                ...
                y = int(self.center_y - pos[1] * self.scale_factor)  # Note: Y is flipped in image coordinates
                points.append((x, y))
            
            # Draw the trajectory line
            for i in range(1, len(points)):
                cv2.line(self.trajectory_image, points[i-1], points[i], (255, 0, 0), 2)
        ...
        if self.trajectory_history:
            ...
            cv2.putText(self.trajectory_image, pos_text, (10, 30), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2)
        
        # Add scale info
        scale_text = f"Scale: 1 grid = {1/self.scale_factor:.1f} m"
        cv2.putText(self.trajectory_image, scale_text, (10, 580), 
    ...
    def update_map_points_image(self):
        ...
        # Draw coordinate axes
        cv2.line(self.map_points_image, (self.center_x, self.center_y), 
                (self.center_x + 50, self.center_y), (0, 0, 255), 2)  # X-axis (red)
        cv2.line(self.map_points_image, (self.center_x, self.center_y), 
                (self.center_x, self.center_y - 50), (0, 255, 0), 2)  # Y-axis (green)
        ...
        if len(self.trajectory_history) > 1:
            ...
            # Draw the trajectory line
            for i in range(1, len(points)):
                cv2.line(self.map_points_image, points[i-1], points[i], (255, 0, 0), 1)
        ...
        # Add legend
        cv2.putText(self.map_points_image, "Map Points (Top-Down View)", (10, 30), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2)
        cv2.putText(self.map_points_image, f"Points: {len(self.map_points)}", (10, 60), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2)
    ...
    def process_frame(self, frame):
        ...
        if self.prev_des is not None and des is not None:
            ...
            if self.show_2d and len(matches) > 0:
                img_matches = cv2.drawMatches(self.prev_frame, self.prev_kp, gray, kp, matches[:50], None,
            ...
            if len(pts1) > 0 and len(pts2) > 0:
                ...
                if R is not None and t is not None:
                    ...
                    cv2.putText(frame, features_text, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 
                                0.6, (0, 255, 0), 2)
...
def parse_args():
    ...
    parser.add_argument('-s', '--scale', type=float, default=1.0, help='Scale factor for trajectory')
    parser.add_argument('--no2d', action='store_true', help='Disable 2D feature visualization')
