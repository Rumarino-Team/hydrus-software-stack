...
class GateMission(BaseMission):
    ...
    def __init__(self):
        ...
        self.previous_orientations = (
            []
        )  # Track previous orientations to avoid duplicates
    ...
    def update_phase(self):
        ...
        if self.current_phase_index < len(self.phases):
            ...
            if self.current_phase == "Search for Gate" and self.search_mission_object(
                "gate"
            ):
                self.current_phase_index += 1
                self.gate_detected = True
            elif (
                self.current_phase == "Identify Images"
                and self.shark_side
                and self.sawfish_side
            ):
                self.current_phase_index += 1
            elif self.current_phase == "Choose Side" and self.chosen_animal:
                self.current_phase_index += 1
            elif (
                self.current_phase == "Approach Gate" and self.is_positioned_for_gate()
            ):
                self.current_phase_index += 1
    ...
    def build_mission_tree(self) -> MissionTreeNode:
        ...
        identify_images = MissionTreeNode(
            task=TaskType.IDENTIFY_TARGET, name="Identify Images"
        )
        ...
        approach_gate = MissionTreeNode(
            task=TaskType.MOVE_TO_CENTER, name="Approach Gate"
        )
        ...
        navigate_gate = MissionTreeNode(
            task=TaskType.MOVE_TO_CENTER, name="Navigate Gate with Style"
        )
    ...
    def create_approach_gate_action(self):
        ...
        def action():
            ...
            approach_point.y = (
                self.divider_position.y + approach_y_offset
            )  # Offset to chosen side
    ...
    def create_navigate_with_style_action(self):
        ...
        def action():
            ...
            if not self.current_style:
                ...
                self.current_style = np.random.choice(
                    styles, p=[0.4, 0.4, 0.2]
                )  # Higher probability for roll and pitch
                rospy.loginfo(
                    f"[Gate] Navigating through gate with style: {self.current_style}"
                )
            ...
            if self.current_style and self.submarine_pose:
                ...
                self.controller_client.send_goal(
                ...
                if not self.is_duplicate_orientation(euler):
                    self.previous_orientations.append(euler)
                    self.style_points += (
                        1 if self.current_style == "yaw" else 2
                    )  # More points for roll and pitch
    ...
    def is_duplicate_orientation(self, new_orientation, tolerance=math.pi / 4):
        ...
        for prev_orientation in self.previous_orientations:
            ...
            if all(
                abs(n - p) < tolerance
                for n, p in zip(new_orientation, prev_orientation)
            ):
                return True
        return False
    ...
    def run(self):
        ...
        if self.mission_tree_root:
            ...
            if self.mission_tree_root.execute():
                rospy.loginfo(
                    f"[Gate] Mission complete! Chosen animal: {self.chosen_animal}, Style points: {self.style_points}"
                )
                self.completed = True
                self.deactivate()
        else:
