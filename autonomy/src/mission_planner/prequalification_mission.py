...
class PreQualificationMission(BaseMission):
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "completion_percentage": 0 if not self.mission_tree_root else
                                     100 if self.mission_tree_root.completed else 
                                     self.mission_tree_root.get_progress() * 100
        }
    ...
    def run(self):
        ...
        status_msg.data = f"{self.name} Mission: Progress {status_dict['completion_percentage']:.1f}%, " \
                         f"Detected: {', '.join(status_dict['objects_detected'])}"
        self.status_pub.publish(status_msg)

        if self.mission_tree_root and not self.mission_tree_root.completed and self.active:
