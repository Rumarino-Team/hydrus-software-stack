...
class SlalomMission(BaseMission):
    ...
    def __init__(self, enable_action_client=True):
        ...
        if enable_action_client:
            self.controller_client = actionlib.SimpleActionClient(
                "/navigate_to_waypoint", NavigateToWaypointAction
            )
    ...
    def detect_gate_sides(self, gate_number: int) -> dict:
        ...
        if len(red_pipes) < 1 or len(white_pipes) < 2:
            rospy.logwarn(
                f"Insufficient detections for gate {gate_number}: {len(red_pipes)} red, {len(white_pipes)} white"
            )
            return {}
        ...
        right_pipe = min(
            right_pipes, key=lambda p: p.point.y
        )  # Leftmost of right pipes
        ...
        gate_sides["center"] = Point(
            x=red_pipe.point.x, y=red_pipe.point.y, z=red_pipe.point.z
        )

        rospy.loginfo(
            f"Gate {gate_number + 1} sides detected - Left: ({gate_sides['left_side'].y:.2f}), "
    ...
    def detect_gate(self, gate_number: int) -> bool:
        ...
        if gate_sides:
            ...
            if gate_number >= len(self.gate_positions):
                ...
                self.gate_positions[gate_number].update(
                    {
                        ...
                        "center": gate_sides["center"],
                        "target_point": gate_sides["left_side"],
                        "detection_count": self.gate_positions[gate_number].get(
                            "detection_count", 0
                        )
                        + 1,
                        "last_updated": rospy.Time.now(),
                ...
                rospy.loginfo(
                    f"Gate {gate_number + 1} position updated (moved {distance_moved:.2f}m)"
                )
    ...
    def navigate_gate(self, gate_number: int) -> bool:
        ...
        rospy.loginfo(
            f"Navigating through left side of gate {gate_number + 1} at "
    ...
    def execute_search_for_gate(self) -> bool:
        ...
        if self.current_waypoint_index < len(self.waypoints):
            if self.controller_client:
                ...
                rospy.loginfo(
                    f"Searching at waypoint {self.current_waypoint_index + 1} (simulated)"
                )
    ...
    def build_mission_tree(self) -> MissionTreeNode:
        ...
        gate1_branch = MissionTreeNode(
            task=TaskType.HOLD_POSITION, name="Gate 1", action=lambda: True
        )
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "completion_percentage": (
                (self.gates_passed / self.total_gates) * 100.0
