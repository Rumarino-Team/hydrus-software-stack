...
class SlalomMissionTester:
    def __init__(self):
        ...
        self.detection_pub = rospy.Publisher(
            "/vision/detections", Detections, queue_size=10
        )
    ...
    def publish_mock_gate_horizontal(
        self, gate_number, gate_x_position, gate_y_position
    ):
        ...
        detections.detections = [
            self.create_detection(
                1, gate_x_position, gate_y_position - 1.0, -2.0
            ),  # White right side
            self.create_detection(
                2, gate_x_position, gate_y_position, -2.0
            ),  # Red center
            self.create_detection(
                1, gate_x_position, gate_y_position + 1.0, -2.0
            ),  # White left side
        ...
        rospy.loginfo(
            f"Published horizontal gate {gate_number + 1} at x={gate_x_position}, y={gate_y_position}"
        )
    ...
    def simulate_submarine_movement(self, start_pos, target_pos, steps=15):
        ...
        rospy.loginfo(
            f"Moving from ({start_pos[0]:.1f}, {start_pos[1]:.1f}) to ({target_pos.x:.1f}, {target_pos.y:.1f})"
        )
        ...
        for i in range(steps + 1):
            ...
            if i == 0:
                rospy.loginfo(
                    f"  Starting at: ({current_x:.1f}, {current_y:.1f}, {current_z:.1f})"
                )
            elif i == steps:
                rospy.loginfo(
                    f"  Reached target: ({current_x:.1f}, {current_y:.1f}, {current_z:.1f})"
                )
            elif i % 3 == 0:  # Log every 3rd step
                rospy.loginfo(
                    f"  Progress {progress*100:.0f}%: ({current_x:.1f}, {current_y:.1f}, {current_z:.1f})"
                )
    ...
    def navigate_through_gate_controlled(self, submarine_pos, gate_data, gate_num):
        ...
        for waypoint_name, waypoint in waypoints:
            ...
            if waypoint_name == "Gate Center":
                rospy.loginfo(
                    f"   Passing BETWEEN red pipe (Y={red_center.y:.2f}) and white left pipe"
                )
                rospy.loginfo(
                    f"   Target Y={waypoint.y:.2f} is ABOVE red center (safe passage)"
                )

            current_pos = self.simulate_submarine_movement(
                current_pos, waypoint, steps=10
            )
        ...
        rospy.loginfo(
            f"   Final position: ({current_pos[0]:.2f}, {current_pos[1]:.2f}, {current_pos[2]:.2f})"
        )
    ...
    def test_complete_slalom_navigation(self):
        ...
        for gate_num, gate_info in enumerate(gate_positions):
            ...
            if self.mission.detect_gate(gate_num):
                ...
                rospy.loginfo(
                    f"   Red pipe center: ({gate_data['center'].x:.1f}, {gate_data['center'].y:.1f})"
                )
                rospy.loginfo(
                    f"   LEFT side target: ({gate_data['left_side'].x:.1f}, {gate_data['left_side'].y:.1f})"
                )
                rospy.loginfo(
                    f"   RIGHT side: ({gate_data['right_side'].x:.1f}, {gate_data['right_side'].y:.1f})"
                )
                ...
                approach_target.x = (
                    gate_data["center"].x - 6.0
                )  # 6m before gate (increased from 4m)
                approach_target.y = (
                    gate_data["left_side"].y + 0.2
                )  # Slightly above left target
                approach_target.z = gate_data["left_side"].z

                submarine_pos = self.simulate_submarine_movement(
                    submarine_pos, approach_target
                )
                rospy.loginfo(f"Reached initial approach position")

                # 3. CONTROLLED NAVIGATION through gate
                submarine_pos = self.navigate_through_gate_controlled(
                    submarine_pos, gate_data, gate_num
                )
                ...
                if self.mission.navigate_gate(gate_num):
                    ...
                    rospy.loginfo(
                        f"SUCCESS! Gate {gate_num + 1} passed with CONTROLLED navigation"
                    )
                    rospy.loginfo(
                        f"   Gates passed: {successful_passages}/{len(gate_positions)}"
                    )
                else:
                    rospy.logwarn(
                        f"Failed to register gate passage for Gate {gate_num + 1}"
                    )
        ...
        rospy.loginfo(
            f"   Final position: ({submarine_pos[0]:.1f}, {submarine_pos[1]:.1f}, {submarine_pos[2]:.1f})"
        )
        ...
        if status["gates_passed"] == len(gate_positions):
            ...
            rospy.logwarn(
                f"Mission incomplete: {status['gates_passed']}/{len(gate_positions)} gates passed"
            )
    ...
    def test_direct_detection(self):
        ...
        if self.mission.detect_gate(0):
            ...
            rospy.loginfo(
                f"  LEFT side: ({gate['left_side'].x:.1f}, {gate['left_side'].y:.1f})"
            )
            rospy.loginfo(f"  Center: ({gate['center'].x:.1f}, {gate['center'].y:.1f})")
            rospy.loginfo(
                f"  RIGHT side: ({gate['right_side'].x:.1f}, {gate['right_side'].y:.1f})"
            )
            rospy.loginfo(
                f"  Submarine will use CONTROLLED navigation through LEFT side"
            )
        else:
    ...
    def test_mission_subscribers(self):
        ...
        has_detection_sub = hasattr(self.mission, "detection_sub") or hasattr(
            self.mission, "detections_sub"
        )
        has_pose_sub = hasattr(self.mission, "pose_sub") or hasattr(
            self.mission, "submarine_pose_sub"
        )
    ...
    def run_test(self):
        ...
        rospy.loginfo(
            "CONTROLLED Horizontal slalom mission integration test starting in 3 seconds..."
        )
        time.sleep(3.0)
        ...
        if not subscribers_ok:
            rospy.logwarn(
                "Missing subscribers but continuing with controlled navigation test..."
            )
        ...
        if success:
            rospy.loginfo(
                "OVERALL SUCCESS: Controlled navigation prevents white pipe collision!"
            )
        else:
