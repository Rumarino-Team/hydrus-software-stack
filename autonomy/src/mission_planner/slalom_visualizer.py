...
class SlalomASCIIVisualizer:
    ...
    def create_frame(self):
        ...
        if self.submarine_pose:
            screen_x, screen_y = self.world_to_screen(
                self.submarine_pose.pose.position.x, self.submarine_pose.pose.position.y
            )
            if 0 < screen_x < self.width - 1 and 0 < screen_y < self.height - 1:
                # Animated submarine
    ...
    def create_status_panel(self):
        ...
        if self.mission_status:
            lines.append(
                f"Gates passed: {self.mission_status.get('gates_passed', 0)}/{self.mission_status.get('total_gates', 3)}"
            )
            lines.append(
                f"Completion: {self.mission_status.get('completion_percentage', 0):.1f}%"
            )
            lines.append(
                f"Current gate: {self.mission_status.get('current_gate', 0) + 1}"
            )
        ...
        if self.current_detections:
            ...
            white_count = sum(
                1 for d in self.current_detections.detections if d.cls == 1
            )
