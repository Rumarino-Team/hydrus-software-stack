...
class MissionManager:
    ...
    def __init__(self):
        ...
        self.status_pub = rospy.Publisher(
            "/mission_manager/status", String, queue_size=10
        )
        ...
        rospy.Service(
            "/mission_manager/select_mission", SetBool, self.handle_select_mission
        )
        ...
        rospy.loginfo(
            "Mission Manager initialized. Available missions: "
                ...
                if dependency in self.missions:
                    ...
                    if dependency == "gate" and mission_name == "tagging":
                        ...
                        if gate_mission.completed:
                            ...
                            rospy.loginfo(
                                f"Applied dependency: Tagging mission will target {chosen_animal}"
                            )
                        else:
                            rospy.logwarn(
                                f"Dependency '{dependency}' for mission '{mission_name}' is not completed"
                            )
                else:
                    rospy.logwarn(
                        f"Dependency '{dependency}' for mission '{mission_name}' not found"
                    )
    ...
    def update_status(self):
        ...
        status_dict = {
            "active_mission": (
                self.active_mission.name if self.active_mission else "None"
            ),
    ...
    def handle_start(self, req):
        ...
        return TriggerResponse(
            success=success,
            message=(
                f"Started mission: {self.active_mission.name}"
                if success
    ...
    def handle_stop(self, req):
        ...
        return TriggerResponse(
            success=success,
            message=(
                f"Stopped mission: {self.active_mission.name}"
                if success
                else "Failed to stop mission"
            ),
        )
    ...
    def handle_reset(self, req):
        ...
        return TriggerResponse(
            success=success,
            message=(
                f"Reset mission: {self.active_mission.name}"
                if success
                else "Failed to reset mission"
            ),
        )
    ...
    def handle_select_mission(self, req):
        ...
        return SetBoolResponse(
            success=success,
            message=(
                f"Selected mission: {mission_name}"
                if success
                else f"Failed to select mission: {mission_name}"
            ),
        )
