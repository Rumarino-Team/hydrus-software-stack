...
class TaggingMission(BaseMission):
    ...
    def __init__(self):
        ...
        self.target_animal = (
            "reef_shark"  # Default target - will be updated from gate mission
        )
        ...
        rospy.loginfo(
            f"[Tagging] Waiting for torpedo service '{self.torpedo_service_name}'..."
        )
        try:
            rospy.wait_for_service(self.torpedo_service_name, timeout=5.0)
            self.fire_torpedo_service = rospy.ServiceProxy(
                self.torpedo_service_name, FireTorpedo
            )
            rospy.loginfo(f"[Tagging] Connected to torpedo service")
        except rospy.ROSException:
            rospy.logwarn(
                f"[Tagging] Torpedo service not available - will retry when needed"
            )
    ...
    def build_mission_tree(self) -> MissionTreeNode:
        ...
        detect_config = MissionTreeNode(
            task=TaskType.IDENTIFY_TARGET, name="Detect Board Configuration"
        )
        detect_config.action = self.create_action(
            TaskType.IDENTIFY_TARGET, "board_configuration"
        )
        search_board.add_child(detect_config)

        # Phase 3: Detect far distance markers
        detect_distance = MissionTreeNode(
            task=TaskType.IDENTIFY_TARGET, name="Detect Far Distance"
        )
        detect_distance.action = self.create_action(
            TaskType.IDENTIFY_TARGET, "horizontal_bar"
        )
        detect_config.add_child(detect_distance)

        # Phase 4: Move to optimal firing position
        approach_board = MissionTreeNode(
            task=TaskType.APPROACH_TARGET, name="Approach Board"
        )
        ...
        identify_first = MissionTreeNode(
            task=TaskType.IDENTIFY_TARGET, name=f"Identify {self.target_animal} Opening"
        )
        identify_first.action = self.create_action(
            TaskType.IDENTIFY_TARGET, "target_opening"
        )
        approach_board.add_child(identify_first)

        # Phase 6: Fire first torpedo
        fire_first = MissionTreeNode(
            task=TaskType.FIRE_TORPEDO, name=f"Fire at {self.target_animal} Opening"
        )
        ...
        identify_second = MissionTreeNode(
            task=TaskType.IDENTIFY_TARGET, name=f"Identify {self.second_animal} Opening"
        )
        identify_second.action = self.create_action(
            TaskType.IDENTIFY_TARGET, "second_opening"
        )
        fire_first.add_child(identify_second)

        # Phase 8: Fire second torpedo
        fire_second = MissionTreeNode(
            task=TaskType.FIRE_TORPEDO, name=f"Fire at {self.second_animal} Opening"
        )
    ...
    def detect_far_distance(self) -> bool:
        ...
        if bars:
            ...
            if board:
                ...
                bar_distances = [
                    self.calculate_distance(bar.position, board.position)
                    for bar in bars
                ]
                self.far_distance = max(bar_distances)  # Use the farthest bar
                rospy.loginfo(
                    f"[Tagging] Far distance detected: {self.far_distance:.2f}m"
                )
                return True
        ...
        if len(self.openings_detected) >= 2:
            ...
            if reef_shark and sawfish:
                ...
                for opening in self.openings_detected:
                    dist_to_shark = self.calculate_distance(
                        opening.position, reef_shark.position
                    )
                    dist_to_sawfish = self.calculate_distance(
                        opening.position, sawfish.position
                    )
    ...
    def fire_torpedo(self) -> bool:
        ...
        try:
            ...
            if response.success:
                rospy.loginfo(
                    f"[Tagging] Torpedo {torpedo_number} fired: {response.message}"
                )
                ...
                rospy.logerr(
                    f"[Tagging] Failed to fire torpedo {torpedo_number}: {response.message}"
                )
                return False
    ...
    def update_phase(self):
        ...
        if self.current_phase_index < len(self.phases):
            ...
            if (
                self.current_phase == "Search for Board"
                and "board" in self.detected_objects
            ):
                phase_complete = True
            elif (
                self.current_phase == "Detect Board Configuration"
                and self.board_configuration
            ):
                phase_complete = True
            elif self.current_phase == "Detect Far Distance" and self.far_distance:
                phase_complete = True
            elif (
                self.current_phase == "Approach Board"
                and self.current_distance >= self.far_distance - 0.1
            ):
                phase_complete = True
            elif (
                self.current_phase == "Identify Target Opening" and self.target_opening
            ):
                phase_complete = True
            elif self.current_phase == "Fire First Torpedo" and self.first_target_hit:
                phase_complete = True
            elif (
                self.current_phase == "Identify Second Opening" and self.second_opening
            ):
                phase_complete = True
