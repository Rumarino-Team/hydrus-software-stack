...
class ManualMissionController:
    def __init__(self):
        ...
        min_duration = (distance_total / self.params['submarine_speed']) + 10.0  # More buffer time
    ...
    def get_object(self, name):
        ...
        if name == "gate" and t > 0.5:
            return MissionObject(
                self.params['gate_position'],
                "gate",
                self.params['gate_confidence']
            )
        ...
        if name in ["reef_shark", "sawfish"] and t > 2.0:
            pos = Point()
            pos.x = self.params['gate_position'].x
            pos.y = self.params['gate_position'].y + (0.5 if name == "reef_shark" else -0.5)
            pos.z = self.params['gate_position'].z
        ...
        if time_to_gate < t < (time_to_gate + 3.0):
            ...
            if self.last_orientation is not None:
                ...
                if current_orientation not in self.visited_orientations:
                    ...
                    pitch_change = abs(current_orientation[1] - self.last_orientation[1])
    ...
    def run(self):
        ...
        rospy.loginfo(f"Final Position: ({final_pose.pose.position.x:.2f}, {final_pose.pose.position.y:.2f}, {final_pose.pose.position.z:.2f})")
