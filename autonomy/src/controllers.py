...
class ProportionalController:
    ...
    class Constants:
        ...
        TORPEDO_FIRE_DURATION: float = (
            1.0  # Time in seconds to keep the torpedo motors at fire PWM
        )
    ...
    def __init__(self):
        ...
        for i in range(self.const.TOTAL_THRUSTERS):
            self.thruster_pubs[i] = rospy.Publisher(
                f"/hydrus/thrusters/{i+1}", Int16, queue_size=10
            )
        ...
        self.target_pub = rospy.Publisher(
            "/controller/target_point", Point, queue_size=10
        )
        self.moving_state_pub = rospy.Publisher(
            "/controller/moving_state", Int16MultiArray, queue_size=10
        )
        self.target_distance_pub = rospy.Publisher(
            "/controller/target_distance", Float32, queue_size=10
        )
        self.original_distance_pub = rospy.Publisher(
            "/controller/original_target_distance", Float32, queue_size=10
        )
        self.distance_from_start_pub = rospy.Publisher(
            "/controller/distance_from_start", Float32, queue_size=10
        )
        ...
        self.server = actionlib.SimpleActionServer(
            "controller_action", NavigateToWaypointAction, self.execute_callback, False
        )
        self.server.start()
    ...
    def handle_fire_torpedo(self, req):
        ...
        try:
            ...
            return FireTorpedoResponse(
                success=True, message=f"Torpedo {torpedo_number} fired successfully"
            )

        except Exception as e:
            error_msg = f"Failed to fire torpedo {torpedo_number}: {str(e)}"
            rospy.logerr(error_msg)
            return FireTorpedoResponse(success=False, message=error_msg)
    ...
    def execute_callback(self, goal):
        ...
        rospy.loginfo(
            f"Goal received: target_point=({target_point.x}, {target_point.y}, {target_point.z})"
        )
        ...
        while not rospy.is_shutdown():
            ...
            if not original_recorded:
                original_distance = self.calculate_distance(
                    self.submarine_pose.pose.position, target_point
                )
            ...
            distance_from_start = self.calculate_distance(
                self.submarine_pose.pose.position, original_point
            )
            ...
            feedback.success = (
                distance < self.const.DISTANCE_THRESHOLD
            )  # Update feedback so you know how close it is
    ...
    def _publish_all(self):
        ...
        depth_pwm = int(
            sum(self.thruster_values[m - 1] for m in self.const.DEPTH_MOTORS_ID)
            / len(self.const.DEPTH_MOTORS_ID)
        )
        self.depth_pub.publish(Int16(depth_pwm))

        # Torpedo motors - send average of torpedo motor values
        torpedo_pwm = int(
            sum(self.thruster_values[m - 1] for m in self.const.TORPEDO_MOTORS_ID)
            / len(self.const.TORPEDO_MOTORS_ID)
        )
        ...
        if self.target_point is not None:
            self.target_pub.publish(
                Point(self.target_point.x, self.target_point.y, self.target_point.z)
            )

        # Publish moving state
        self.moving_state_pub.publish(
            Int16MultiArray(data=[int(m) for m in self.moving])
        )

        # Publish delta value (distance to target)
        distance = (
            self.calculate_distance(
                self.submarine_pose.pose.position, self.target_point
            )
            if self.submarine_pose and self.target_point
            else 0.0
        )
    ...
    def _publish_cmd_vel(self):
        ...
        fwd_pwm = (
            sum(
                self.thruster_values[m]
                for m in front_motor_indices + back_motor_indices
            )
            / 4.0
        )
        ...
        left_pwm = (
            sum(
                self.thruster_values[m]
                for m in [front_motor_indices[0], back_motor_indices[0]]
            )
            / 2.0
        )
        right_pwm = (
            sum(
                self.thruster_values[m]
                for m in [front_motor_indices[1], back_motor_indices[1]]
            )
            / 2.0
        )
    ...
    def adjust_depth_motors(self, current_pose, target_point):
        ...
        rospy.loginfo(
            f"Depth adjustment: current={position.z:.2f}, target={target_point.z:.2f}, distance={depth_distance:.2f}"
        )
        ...
        if depth_distance > self.const.DISTANCE_THRESHOLD:
            if dz > 0:  # Need to go up
                for motor_id in self.const.DEPTH_MOTORS_ID:
                    ...
                    self.thruster_values[idx] = (
                        self.const.PWM_NEUTRAL + self.const.DEPTH_PWM_ADJUST
                    )
                    rospy.loginfo(
                        f"Setting depth motor {motor_id} to upward: {self.const.PWM_NEUTRAL + self.const.DEPTH_PWM_ADJUST}"
                    )
                ...
                for motor_id in self.const.DEPTH_MOTORS_ID:
                    ...
                    rospy.loginfo(
                        f"Setting depth motor {motor_id} to downward: {self.const.PWM_NEUTRAL - self.const.DEPTH_PWM_ADJUST}"
                    )
    ...
    def adjust_rotation_motors(self, current_pose, target_point):
        ...
        rospy.loginfo(
            f"Rotation adjustment: target_yaw={target_yaw:.2f}, current_yaw={current_yaw:.2f}, diff={angle_diff:.2f}"
        )
            ...
            if angle_diff > 0:
                ...
                for motor_id in self.const.FRONT_MOTORS_ID:
                    ...
                    if idx == self.const.FRONT_MOTORS_ID[0] - 1:
                        self.thruster_values[idx] = (
                            self.const.PWM_NEUTRAL - self.const.ROTATION_PWM_ADJUST
                        )
                    else:
                        self.thruster_values[idx] = (
                            self.const.PWM_NEUTRAL + self.const.ROTATION_PWM_ADJUST
    ...
    def adjust_linear_motors(self, current_pose, target_point):
        ...
        rospy.loginfo(
            f"Linear movement: distance={distance:.2f}, Distance Threshold={self.const.DISTANCE_THRESHOLD}"
        )
        ...
        if distance > self.const.DISTANCE_THRESHOLD:
            ...
            for motor_id in self.const.FRONT_MOTORS_ID:
                idx = motor_id - 1  # Convert 1-based to 0-based index
                self.thruster_values[idx] = (
                    self.const.PWM_NEUTRAL + self.const.LINEAR_PWM_ADJUST
                )
                rospy.loginfo(
                    f"Setting front motor {motor_id} to forward: {self.const.PWM_NEUTRAL + self.const.LINEAR_PWM_ADJUST}"
                )
            ...
            for motor_id in self.const.BACK_MOTORS_ID:
                ...
                rospy.loginfo(
                    f"Setting back motor {motor_id} to forward: {self.const.PWM_NEUTRAL + self.const.LINEAR_PWM_ADJUST}"
                )
    ...
    def calculate_yaw_to_target(current_position, target_position):
        ...
        target_x = (
            target_position.x
            if hasattr(target_position, "x")
            else target_position.position.x
        )
        target_y = (
            target_position.y
            if hasattr(target_position, "y")
            else target_position.position.y
        )
