...
def example_3d_bounding_box_usage():
    ...
    detection = custom_types.Detection(
        x1=200, y1=150, x2=400, y2=350,  # 2D bounding box
        cls=0, conf=0.9
    )
    ...
    if detection.bbox_3d is not None:
        ...
        print(f"Size: {detection.bbox_3d.width:.2f} x {detection.bbox_3d.height:.2f} x {detection.bbox_3d.depth:.2f} meters")
        ...
        print(f"Legacy centroid: ({detection.point.x:.2f}, {detection.point.y:.2f}, {detection.point.z:.2f}) meters")
    else:
